...
public class BinarySearchTree<E extends Comparable<E>> {
    ...
    public void inorderSupport(BSTNode<E> root, StringBuilder inorder) {
        if(root != null) {
	...
	public String levelorder() {
		...
		levelorderSupport(root, levelorderTraversal);
		return levelorderTraversal.toString();
	} // end levelorder
	
	private boolean printlevel(BSTNode<E> root, int level, StringBuilder levelorder) {
		...
		boolean left = printlevel(root.getLeftChild(), level-1, levelorder);
		boolean right = printlevel(root.getRightChild(), level-1, levelorder);
		
		return left || right;
	}
	
	private void levelorderSupport(BSTNode<E> root, StringBuilder levelorder) {
		int level = 1;
		while (printlevel(root, level, levelorder)) {
			level++;
		}
	}
} // end class
